...
function [initial_values,senstivity_matrix] = read_general_sensitivity(filename,DESVAR_labels,DRESP1_ID,R_TYPE)
...
while currentFPI<EOF
    ...
    if length(linebdf)>120 && isempty(findstr(linebdf,keywords2)) == 0  && isempty(findstr(linebdf,keywords3)) == 0  && isempty(findstr(linebdf,R_TYPE)) == 0
        ...
        while length(linebdf)>1 && isempty(dresp1_index )==0
            ...
            if length(linebdf)>45 && isempty(str2num(linebdf(37:45))) == 0
                ...
                if isempty(str2num(linebdf(14:24))) == 0
                    ...
                    initial_values = [initial_values ;str2num(linebdf(14:24))];
...
%                 senstivity_matrix
%                 stress_sens_temp
%                 size(senstivity_matrix)
%                 size(stress_sens_temp)
                senstivity_matrix =[ senstivity_matrix ;   stress_sens_temp];
